...
function parseOpenGraph(html: string, url: string): OGData {
  const data: OGData = {};
...
export async function getCachedOG(env: any, url: string): Promise<OGData | null> {
  try {
...
export async function setCachedOG(env: any, url: string, data: OGData): Promise<void> {
  try {
    const cacheKey = `og_cache:${url}`;
