...
generators.post('/:slug', async (c) => {
  ...
  `).bind(slug).first();
    ...
    `).bind(runId, generator.id, auth?.userId || null, fingerprint, JSON.stringify(data.inputs), output_text, JSON.stringify(data.related_terms || []), data.made_public ? 1 : 0).run();
      ...
      `).bind(nanoid(), runId, new Date().toISOString(), `Generated ${generator.name}: ${output_text.substring(0, 100)}...`).run();
        }
        return c.json({
            output_text,
            run_id: runId,
...
function hashInputs(inputs, options) {
