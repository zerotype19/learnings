...
router.post('/', async (c) => {
  try {
    ...
    const existingVote = await c.env.DB.prepare(
      'SELECT id FROM votes_v2 WHERE entity_type = ? AND entity_id = ? AND user_id = ?'
    ).bind(entity_type, entity_id, fingerprint).first();
    ...
    await c.env.DB.prepare(
      'INSERT INTO votes_v2 (id, entity_type, entity_id, user_id, created_at) VALUES (?, ?, ?, ?, ?)'
    ).bind(voteId, entity_type, entity_id, fingerprint, now).run();
    ...
    if (entity_type === 'wall') {
      ...
      if (post) {
        ...
        ).bind(newVoteCount, hotScore, now, entity_id).run();
      ...
      const entry = await c.env.DB.prepare(
        'SELECT votes FROM challenge_entries_v2 WHERE id = ?'
      ).bind(entity_id).first() as any;
      ...
      // Handle term links
      await c.env.DB.prepare(
        'UPDATE term_links SET votes = votes + 1 WHERE id = ?'
      ).bind(entity_id).run();
      
      const link = await c.env.DB.prepare(
        'SELECT votes FROM term_links WHERE id = ?'
      ).bind(entity_id).first() as any;
