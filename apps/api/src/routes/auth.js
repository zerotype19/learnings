...
auth.post("/magic", async (c) => {
    ...
    if (!email)
        return c.text("Email required", 400);
    const tok = nanoid(32);
    const hash = await sha256hex(tok);
    await c.env.DB.prepare("INSERT INTO auth_tokens (id,email,token_hash,action,meta_json,created_at) VALUES (?,?,?,?,?,?)").bind(nanoid(), email.toLowerCase(), hash, action, JSON.stringify(meta), Date.now()).run();
...
auth.get("/callback", async (c) => {
    ...
    const row = await c.env.DB.prepare("SELECT * FROM auth_tokens WHERE token_hash=? AND used_at IS NULL ORDER BY created_at DESC LIMIT 1")
        .bind(hash).first();
    if (!row)
        return c.text("Invalid or used link", 400);
    // expire after 15m
    if (Date.now() - row.created_at > 15 * 60 * 1000)
        return c.text("Link expired", 400);
    // find or create user
    let user = await c.env.DB.prepare("SELECT * FROM users WHERE email=?").bind(row.email).first();
    if (!user) {
        const id = nanoid();
        const handle = row.email.split("@")[0].replace(/[^a-z0-9]+/g, "-").slice(0, 24) + "-" + Math.floor(Math.random() * 1000);
        await c.env.DB.prepare("INSERT INTO users (id,email,handle,display_name,role,trust,created_at) VALUES (?,?,?,?, 'member', 0, ?)").bind(id, row.email, handle, handle, Date.now()).run();
...
auth.post("/claim", async (c) => {
    ...
    if (!fingerprint)
        return c.text("Fingerprint required", 400);
...
auth.get("/linkedin/callback", async (c) => {
    ...
    try {
        ...
        if (!email)
            return c.text("Failed to get email", 400);
        ...
        if (!user) {
            ...
            await c.env.DB.prepare("INSERT INTO users (id,email,handle,display_name,role,trust,created_at) VALUES (?,?,?,?, 'member', 0, ?)").bind(id, email, handle, displayName, Date.now()).run();
            user = await c.env.DB.prepare("SELECT * FROM users WHERE id=?").bind(id).first();
        }
