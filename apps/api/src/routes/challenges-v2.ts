...
const challengesV2 = new Hono<{ Bindings: Env }>();
...
challengesV2.get('/', async (c) => {
  try {
    ...
    const items = (results || []).map((challenge: any) => ({
      id: challenge.id,
      slug: challenge.slug,
...
challengesV2.get('/:id/entries', async (c) => {
  ...
  try {
    ...
    const items = (results || []).map((entry: any) => ({
      id: entry.id,
...
challengesV2.post('/:id/entries', async (c) => {
  ...
  try {
    ...
    `).bind(
      ...
      entryId,
      now,
      `New challenge entry: ${data.title || data.body.substring(0, 50)}...`
    ).run();
...
challengesV2.get('/:id/leaderboard', async (c) => {
  ...
  try {
    ...
    const items = await Promise.all((results || []).map(async (user: any) => {
      let bestEntry = null;
