...
router.get('/:slug/related', async (c) => {
  try {
    ...
    if (tags.length === 0) {
      ...
      const relatedTerms = (result.results || []).map((term) => ({
        ...term,
...
router.get('/:slug', async (c) => {
    try {
        const slug = c.req.param('slug');
        const stmt = c.env.DB.prepare('SELECT * FROM terms_v2 WHERE slug = ? AND status = "published"');
        const result = await stmt.bind(slug).first();
        if (!result) {
            return c.json({ error: 'Term not found' }, 404);
        }
        // Increment view count
        await c.env.DB.prepare('UPDATE terms_v2 SET views = views + 1 WHERE id = ?').bind(result.id).run();
        // Parse JSON fields
        const processedItem = {
            ...result,
...
router.post('/submit', async (c) => {
    ...
    const ok = await checkRate(c.env, `ip:${fingerprint}:terms-submit`, 5, 60);
    if (!ok)
    ...
    `);
        await stmt.bind(id, parsed.data.title, null, parsed.data.definition, parsed.data.examples || null, parsed.data.tags ? JSON.stringify(parsed.data.tags) : null, parsed.data.links ? JSON.stringify(parsed.data.links) : null, fingerprint, // submitted_by
        now, now).run();
