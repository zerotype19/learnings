...
router.post('/submit', async (c) => {
    ...
    const ok = await checkRate(c.env, `ip:${fingerprint}:wall-submit`, 3, 60);
    if (!ok)
    ...
    `);
        await stmt.bind(id, parsed.data.title, parsed.data.body || null, parsed.data.source_url, parsed.data.tags ? JSON.stringify(parsed.data.tags) : null, parsed.data.suggested_terms ? JSON.stringify(parsed.data.suggested_terms) : null, fingerprint, // submitted_by
        now).run();
...
router.post('/:id/vote', async (c) => {
    try {
        ...
        const existingVote = await c.env.DB.prepare('SELECT id FROM wall_votes WHERE post_id = ? AND user_id = ?').bind(postId, fingerprint).first();
        if (existingVote) {
        ...
        const voteId = nanoid();
        const now = new Date().toISOString();
        await c.env.DB.prepare('INSERT INTO wall_votes (id, post_id, user_id, created_at) VALUES (?, ?, ?, ?)').bind(voteId, postId, fingerprint, now).run();
        // Update vote count and hot score
        const post = await c.env.DB.prepare('SELECT created_at, vote_count FROM wall_posts WHERE id = ?').bind(postId).first();
        if (post) {
            ...
            await c.env.DB.prepare('UPDATE wall_posts SET vote_count = ?, hot_score = ?, last_activity_at = ? WHERE id = ?').bind(newVoteCount, hotScore, now, postId).run();
