...
wallV2.post('/submit', async (c) => {
  ...
  `).bind(id, data.title, data.body || '', data.source_url, JSON.stringify(data.tags || []), JSON.stringify(data.suggested_terms || []), auth?.userId || fingerprint).run();
    return c.json({
        id,
        status: 'queued',
...
wallV2.post('/vote', async (c) => {
    ...
    `).bind(nanoid(), entity_id, auth?.userId || null, fingerprint).run();
