...
// Main function
function main() {
  console.log('🚀 Starting smart import with variations support...');
  ...
  const migrationPath = path.join(__dirname, '..', 'infra', 'd1-migrations', '049_import_smart_variations.sql');
  fs.writeFileSync(migrationPath, migrationContent);
  
  console.log(`✅ Migration file created: ${migrationPath}`);
  console.log(`📝 Generated SQL for ${terms.length} terms with smart variations`);
  
  // Generate report
  const reportPath = path.join(__dirname, 'smart-variations-report.txt');
  const report = generateReport(terms);
  fs.writeFileSync(reportPath, report);
  
  console.log(`📋 Report generated: ${reportPath}`);
  console.log('🎉 Smart variations import preparation complete!');
  console.log('\nNext steps:');
  console.log('1. Apply schema migration: cd apps/api && wrangler d1 migrations apply learnings --remote');
  console.log('2. Apply terms migration: cd apps/api && wrangler d1 migrations apply learnings --remote');
  console.log('3. Verify the import was successful');
}

// Generate SQL migration
function generateMigration(terms) {
  let sql = `-- Import terms with smart variations support
-- Generated on ${new Date().toISOString()}
-- Total terms: ${terms.length}
-- This will add new definitions for existing terms as variations
...
`;
  ...
  terms.forEach((term, index) => {
    const id = `term_${generateSlug(term.term)}_v2`;
    const slug = `${generateSlug(term.term)}-v2`;
  ...
  id, slug, title, definition, examples, tags, status, views, seq, created_at, updated_at, base_term, variation
) VALUES (
  '${id}',
  '${slug}',
  '${term.term.replace(/'/g, "''")}',
...
function generateReport(terms) {
  let report = `Smart Variations Import Report
