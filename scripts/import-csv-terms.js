const fs = require('fs');
const path = require('path');

// Read the CSV file
const csvPath = path.join(__dirname, '..', 'corporate_buzzwords.csv');
const csvContent = fs.readFileSync(csvPath, 'utf8');

// Parse CSV (simple parser for this specific format)
const lines = csvContent.split('\n').filter(line => line.trim());
const headers = lines[0].split(',').map(h => h.trim());

console.log('Headers:', headers);

// Process each data row
const terms = [];
for (let i = 1; i < lines.length; i++) {
  const line = lines[i];
  if (!line.trim()) continue;
  
  // Simple CSV parsing (handles quoted fields)
  const fields = [];
  let current = '';
  let inQuotes = false;
  
  for (let j = 0; j < line.length; j++) {
    const char = line[j];
    if (char === '"') {
      inQuotes = !inQuotes;
    } else if (char === ',' && !inQuotes) {
      fields.push(current.trim());
      current = '';
    } else {
      current += char;
    }
  }
  fields.push(current.trim());
  
  if (fields.length >= 5) {
    const [term, shortDef, fullDef, example, tags] = fields;
    
    // Generate slug from term name
    const slug = term
      .toLowerCase()
      .replace(/[^a-z0-9\s-]/g, '') // Remove special chars
      .replace(/\s+/g, '-') // Replace spaces with hyphens
      .replace(/-+/g, '-') // Replace multiple hyphens with single
      .replace(/^-|-$/g, ''); // Remove leading/trailing hyphens
    
    // Generate unique ID
    const id = `term_${slug.replace(/-/g, '_')}`;
    
    // Parse tags (remove quotes and split by comma)
    const tagArray = tags
      .replace(/^"|"$/g, '') // Remove surrounding quotes
      .split(',')
      .map(t => t.trim())
      .filter(t => t);
    
    terms.push({
      id,
      slug,
      title: term,
      definition: fullDef,
      short_def: shortDef,
      example: example,
      tags: JSON.stringify(tagArray),
      status: 'published',
      views: Math.floor(Math.random() * 100) + 1, // Random view count
      seq: i // Sequential order
    });
  }
}

console.log(`Processed ${terms.length} terms`);

// Generate SQL INSERT statements
const sqlStatements = terms.map(term => {
  return `INSERT OR IGNORE INTO terms_v2 (
    id, slug, title, definition, short_def, examples, tags, 
    status, views, seq, created_at, updated_at
  ) VALUES (
    '${term.id}',
    '${term.slug}',
    '${term.title.replace(/'/g, "''")}',
    '${term.definition.replace(/'/g, "''")}',
    '${term.short_def.replace(/'/g, "''")}',
    '${term.example.replace(/'/g, "''")}',
    '${term.tags}',
    '${term.status}',
    ${term.views},
    ${term.seq},
    CURRENT_TIMESTAMP,
    CURRENT_TIMESTAMP
  );`;
});

// Write to SQL file
const sqlContent = `-- Import corporate buzzwords from CSV
-- Generated on ${new Date().toISOString()}

PRAGMA foreign_keys=ON;

${sqlStatements.join('\n\n')}

-- Update sequence numbers to be sequential
UPDATE terms_v2 SET seq = (
  SELECT COUNT(*) FROM terms_v2 t2 
  WHERE t2.created_at <= terms_v2.created_at
) WHERE id LIKE 'term_%';

-- Add some sample votes for engagement
INSERT OR IGNORE INTO votes_v2 (id, entity_type, entity_id, user_fingerprint, reaction, created_at)
SELECT 
  'vote_' || substr(hex(randomblob(16)), 1, 8),
  'term',
  t.id,
  'csv_import_' || substr(hex(randomblob(8)), 1, 8),
  CASE (random() % 3)
    WHEN 0 THEN 'cringe'
    WHEN 1 THEN 'heard1000x' 
    ELSE 'chefskiss'
  END,
  CURRENT_TIMESTAMP
FROM terms_v2 t 
WHERE t.id LIKE 'term_%'
LIMIT 50;
`;

const outputPath = path.join(__dirname, '..', 'infra', 'd1-migrations', '033_import_corporate_buzzwords.sql');
fs.writeFileSync(outputPath, sqlContent);

console.log(`Generated SQL file: ${outputPath}`);
console.log(`Total terms to import: ${terms.length}`);

// Show first few terms as preview
console.log('\nPreview of first 5 terms:');
terms.slice(0, 5).forEach(term => {
  console.log(`- ${term.title} (${term.slug})`);
});
