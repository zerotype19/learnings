...
// Main function
function main() {
  console.log('🚀 Starting import of final terms batch (fixed version)...');
  ...
  const migrationPath = path.join(__dirname, '..', 'infra', 'd1-migrations', '046_import_final_terms_fixed.sql');
  ...
  // Generate report
  const reportPath = path.join(__dirname, 'final-terms-fixed-report.txt');
  const report = generateReport(terms);
  fs.writeFileSync(reportPath, report);
  
  console.log(`📋 Report generated: ${reportPath}`);
  console.log('🎉 Final terms import preparation complete!');
  console.log('\nNext steps:');
...
function generateMigration(terms) {
  let sql = `-- Import final batch of corporate buzzwords (fixed version)
...
function generateReport(terms) {
  let report = `Final Terms Import Report (Fixed Version)
...
`;
  ...
  terms.forEach(term => {
    ...
    } else if (tags.includes('management') || tags.includes('leadership')) {
    ...
    } else if (tags.includes('workplace')) {
      categories['Workplace & Culture'].push(term.term);
    } else if (tags.includes('customer')) {
      categories['Customer & Market'].push(term.term);
    } else {
