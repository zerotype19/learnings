...
// Main function
function main() {
  console.log('🚀 Generating variations migration...');
  ...
  const migrationPath = path.join(__dirname, '..', 'infra', 'd1-migrations', '051_import_variations_complete.sql');
  ...
  // Generate report
  const reportPath = path.join(__dirname, 'variations-complete-report.txt');
  const report = generateReport(terms);
  fs.writeFileSync(reportPath, report);
  
  console.log(`📋 Report generated: ${reportPath}`);
  console.log('🎉 Variations migration preparation complete!');
}

// Generate SQL migration
function generateMigration(terms) {
  let sql = `-- Import term variations with unique slugs
...
`;
  ...
  terms.forEach((term, index) => {
    const id = `term_${generateSlug(term.term)}_alt`;
