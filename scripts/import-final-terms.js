...
// Main function
function main() {
  console.log('🚀 Starting import of final terms batch...');
  ...
  const migrationPath = path.join(__dirname, '..', 'infra', 'd1-migrations', '045_import_final_terms.sql');
  ...
  // Generate report
  const reportPath = path.join(__dirname, 'final-terms-report.txt');
...
function generateMigration(terms) {
  let sql = `-- Import final batch of corporate buzzwords
...
function generateReport(terms) {
  let report = `Final Terms Import Report
