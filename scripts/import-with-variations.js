...
// Generate slug from term with variation suffix
function generateSlug(term, variation = 1) {
  const baseSlug = term
    .toLowerCase()
  ...
  return variation > 1 ? `${baseSlug}-v${variation}` : baseSlug;
}
...
// Check if term already exists and get variation number
function getVariationNumber(term, existingTerms) {
  const baseSlug = generateSlug(term);
  let variation = 1;
  
  while (existingTerms.has(`${baseSlug}${variation > 1 ? `-v${variation}` : ''}`)) {
    variation++;
  }
...
// Main function
function main() {
  console.log('🚀 Starting import with term variations support...');
  ...
  const migrationPath = path.join(__dirname, '..', 'infra', 'd1-migrations', '047_import_with_variations.sql');
  fs.writeFileSync(migrationPath, migrationContent);
  
  console.log(`✅ Migration file created: ${migrationPath}`);
  console.log(`📝 Generated SQL for ${terms.length} terms with variations`);
  
  // Generate report
  const reportPath = path.join(__dirname, 'variations-import-report.txt');
  const report = generateReport(terms);
  fs.writeFileSync(reportPath, report);
  
  console.log(`📋 Report generated: ${reportPath}`);
  console.log('🎉 Variations import preparation complete!');
  console.log('\nNext steps:');
...
function generateMigration(terms) {
  let sql = `-- Import terms with variations support
-- Generated on ${new Date().toISOString()}
-- Total terms: ${terms.length}
-- Allows multiple definitions for the same term with different slugs
...
`;
  ...
  // Get the next sequence number (assuming we have 710 existing terms)
  const startSeq = 711;
  
  terms.forEach((term, index) => {
    // Generate variation number (v1, v2, etc.)
    const variation = 1; // For now, always use v1 - in production you'd check existing terms
    const id = `term_${generateSlug(term.term, variation)}`;
    const slug = generateSlug(term.term, variation);
...
function generateReport(terms) {
  let report = `Variations Import Report
Generated: ${new Date().toISOString()}
Total Terms: ${terms.length}

This import allows multiple definitions for the same term by using different slugs.
For example:
- "Bandwidth" (original)
- "Bandwidth-v2" (new definition)
- "Bandwidth-v3" (another perspective)

TERMS TO BE IMPORTED:
`;

  terms.forEach((term, index) => {
    const slug = generateSlug(term.term, 1);
    report += `\n${index + 1}. ${term.term}\n`;
    report += `   Slug: ${slug}\n`;
